...
class TestMotionPlanner(unittest.TestCase):
    ...
    def test_gridworld_distance(self):
        planner = (ml_planner_simple.ml_action_manager.joint_motion_planner.
                   motion_planner)
        start = ((2, 1), e)
        end = ((1, 1), w)
    ...
    def simple_mdp_already_at_goal(self, planner):
        ...
        self.check_single_motion_plan(planner,
                                      start_status,
                                      goal_status,
                                      expected_length=1)
    ...
    def test_larger_mdp(self):
        if large_mdp_tests:
            planner = (ml_planner_large.ml_action_manager.joint_motion_planner.
                       motion_planner)
...
class TestJointMotionPlanner(unittest.TestCase):
    ...
    def check_joint_plan(
        ...
        """Runs the plan in the environment and checks that the intended goals
        are achieved."""
        debug = False
        (
            action_plan,
        ...
        start_state = OvercookedState([P(*start[0]), P(*start[1])], {},
                                      order_list=["any", "any"])
