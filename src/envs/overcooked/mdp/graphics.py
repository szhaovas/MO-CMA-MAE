...
PLAYER_ARROW_ORIENTATION = {
    Direction.DIRECTION_TO_NAME[Direction.NORTH]: (
        (15, 300),
        (35, 300),
        (35, 100),
        (50, 100),
    ...
    Direction.DIRECTION_TO_NAME[Direction.SOUTH]: (
        (15, 0),
        (35, 0),
        (35, 200),
        (50, 200),
    ...
    Direction.DIRECTION_TO_NAME[Direction.EAST]: (
        (0, 15),
        (0, 35),
        (200, 35),
        (200, 50),
    ...
    Direction.DIRECTION_TO_NAME[Direction.WEST]: (
        (300, 15),
        (300, 35),
        (100, 35),
        (100, 50),
...
def get_player_sprite(player, player_index):
    """Returns loaded image of player(aka chef), the player's hat, and the color
    of the array to draw on top of the player.
    ...
    hat_img_path = os.path.join(
        ASSETS_DIR,
        CHEF_DIR,
        f"{orientation_str}-{PLAYER_HAT_COLOR[player_index]}.png",
    )
...
def get_object_sprite(obj, on_pot=False):
    ...
    if not on_pot:
        ...
        obj_img_path = os.path.join(
            ASSETS_DIR,
            OBJECT_DIR,
            f"soup-{soup_type}-{num_items}-cooking.png",
        )
...
def draw_arrow(window, player, player_index, pos, time_left):
    ...
    arrow_position = [[j * shift * time_left
                       for j in i]
...
def get_orientation_str(player):
    ...
    orientation_str = Direction.DIRECTION_TO_NAME[orientation]
...
def render_from_grid(lvl_grid, log_dir, filename):
    ...
    This function is used for visualization the levels generated which are
    possibily broken or invalid. It also does not take the orientation of the
    players into account. So this method should not be used for actual game
    rendering.
    """
    ...
    for y, terrain_row in enumerate(lvl_grid):
        for x, terrain in enumerate(terrain_row):
            ...
            if str.isdigit(terrain):
                from .overcooked_mdp import PlayerState
