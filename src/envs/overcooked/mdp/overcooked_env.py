...
from .overcooked_mdp import OvercookedGridworld, EVENT_TYPES
...
class OvercookedEnv:
    ...
    trajs = {
        ...
        "ep_dones":     [ [traj_1_timestep_dones], [traj_2_timestep_dones], ... ],          # Done values (should be all 0s except last one for each traj) TODO old: add this to traj checks
    ...
    def __init__(
        ...
        """
        Args:
            mdp_generator_fn (callable): A no-argument function that returns a
                OvercookedGridworld instance
            start_state_fn (callable): Function that returns start state for the
                MDP, called at each environment reset
            horizon (int): Number of steps before the environment returns
                done=True
            info_level (int): Change amount of logging
            _variable_mdp (bool): This input should be ignored in nearly all
                cases. Should automatically keep track of whether the mdp
                changes or is constant across episodes.

        TODO old: Potentially make changes based on this discussion
        ...
        if (
            ...
            and self.state.order_list is None
            and self.info_level > 0
        ):
            logger.info(
                "Environment has (near-)infinite horizon and no terminal "
                "states. Reduce info level of OvercookedEnv to not see this "
                "message."
            )
    ...
    def from_mdp(
        ...
        assert _variable_mdp is False, (
            "from_mdp should not be called with _variable_mdp=True. If you "
            "performed this call, most likely there is an inconsistency in the "
            "env_params dictionary that you were trying to use to create the "
            "Environment."
        )
    ...
    def env_params(self):
        """
        Env params should be though of as all of the params of an env WITHOUT
        the mdp. Alone, env_params is not sufficent to recreate a copy of the
        Env instance, but it is together with mdp_params (which is sufficient to
        build a copy of the Mdp instance).
        """
        return {
    ...
    def copy(self):
        # TODO old: Add testing for checking that these util methods are up to
        # date?
        return OvercookedEnv(
    ...
    def print_state_transition(self, a_t, r_t, info):
        ...
        logger.info(
            f"Timestep: {self.t}\nJoint action taken: "
            f"{tuple(Action.ACTION_TO_CHAR[a] for a in a_t)} \t Reward: {r_t} "
            f"+ shape * {info['agent_infos']} \n{self}\n"
        )
    ...
    def step(self, joint_action, joint_agent_action_info=None):
        ...
        On being done, stats about the episode are added to info:
            ep_sparse_r: the environment sparse reward, given only at soup
                delivery
            ep_shaped_r: the component of the reward that is due to reward
                shaped (excluding sparse rewards)
        ...
        (
            ...
            game_stat_infos,
        ) = self.mdp.get_state_transition(self.state, joint_action)
        timestep_sparse_reward, timestep_reward_shaping = (
            sum(sparse_reward_by_agent),
            sum(reward_shaping_by_agent),
        )

        # Update game_stats
        self._update_game_stats(
            sparse_reward_by_agent, reward_shaping_by_agent, game_stat_infos
        )
    ...
    def _prepare_info_dict(
        ...
        """
        The normal timestep info dict will contain infos specifc to each agent's
        action taken, and reward shaping information.
        """
        # Get the agent action info, that could contain info about action probs,
        # or other custom user defined information
        info = {
            "agent_infos": [
                joint_agent_action_info[agent_idx]
                for agent_idx in range(self.mdp.num_players)
            ],
            "shaped_r_by_agent": reward_shaping_by_agent,
            "sparse_r_by_agent": sparse_reward_by_agent,
        }
        return info
    ...
    def _update_game_stats(
        self, sparse_reward_by_agent, reward_shaping_by_agent, game_stat_infos
    ):
        """
        Update the game stats dict based on the events of the current step
        NOTE: the timer ticks after events are logged, so there can be events
        from time 0 to time self.horizon - 1
        """
    ...
    def run_agents(
        ...
        """
        Trajectory returned will a list of state-action pairs (s_t, joint_a_t,
        r_t, done_t, info_t).
        """
        ...
        while not done:
            ...
            # logger.info(f"Time spent: {time.time()-start_time}")
            # tmp = input()
    ...
    def get_rollouts(
        ...
        """
        Simulate `num_games` number rollouts with the current agent_pair and
        returns processed trajectories.

        Returning excessive information to be able to convert trajectories to
        any required format (baselines, stable_baselines, etc)

        metadata_fn returns some metadata information computed at the end of
        each trajectory based on some of the trajectory data.

        NOTE: this is the standard trajectories format used throughout the
        codebase
        """
        ...
        for i in range_iterator:
            ...
            if info:
                ...
                description = (
                    f"Avg rew: {mu:.2f} (std: "
                    f"{np.std(trajectories['ep_returns']):.2f}, "
                    f"se: {se:.2f}); avg len: "
                    f"{np.mean(trajectories['ep_lengths']):.2f}; "
                )
        ...
        # TODO old: should probably transfer check methods over to Env class
    ...
    def get_agent_infos_for_trajectories(trajectories, agent_idx):
        ...
        with as keys the keys returned by the agent in it's agent_info
        dictionary
    ...
    def proportion_stuck_time(trajectories, agent_idx, stuck_time=3):
        """
        Simple util for calculating a guess for the proportion of time in the
        trajectories during which the agent with the desired agent index was
        stuck.
            ...
            for traj_timestep in range(
                ...
                if traj_timestep >= stuck_time:
                    ...
                    if len({item for item in recent_player_pos_and_or}) == 1:
                        # If there is only one item in the last stuck_time
                        # steps, then we classify the agent as stuck
...
class Overcooked(gym.Env):
    ...
    information that is necessary to be able to handle the multi-agent nature of
    the environment. There are probably better ways to handle this, but we found
    this to work with minor modifications to OpenAI Baselines.
    ...
    means that it is necessary to pass on this information in the output to know
    for which agent index featurizations should be made for other agents.

    For example, say one is training A0 paired with A1, and A1 takes a custom
    state featurization. Then in the runner.py loop in OpenAI Baselines, we will
    get the lossless encodings of the state, and the true Overcooked state. When
    we encode the true state to feed to A1, we also need to know what agent
    index it has in the environment (as encodings will be index dependent).
    """
    ...
    def custom_init(self, base_env, featurize_fn, baselines_reproducible=False):
        ...
        featurize_fn(mdp, state): fn used to featurize states returned in the
        'both_agent_obs' field
        """
        if baselines_reproducible:
            # NOTE:
            # This will cause all agent indices to be chosen in sync across
            # simulation envs (for each update, all envs will have index 0 or
            # index 1).
            # This is to prevent the randomness of choosing agent indexes from
            # leaking when using subprocess-vec-env in baselines (which seeding
            # does not reach) i.e. having different results for different runs
            # with the same seed.
            # The effect of this should be negligible, as all other randomness
    ...
    def _setup_observation_space(self):
        ...
        high = np.ones(obs_shape) * max(
            dummy_mdp.soup_cooking_time, dummy_mdp.num_items_for_soup, 5
        )
    ...
    def reset(self):
        ...
        agents are trained to be able to complete the task starting at either of
        the hardcoded positions.
